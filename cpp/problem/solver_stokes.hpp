...
/// @param delta macro element delta parameter
/// @return vector containing data of the velocity and the pressure
std::vector<double> solve(GFESpace<Mesh2> &Vh, GFESpace<Mesh2> &Ph, FunFEM<Mesh2> &fh, FunFEM<Mesh2> &mu,
                          std::vector<std::pair<FunFEM<Mesh2> *, std::list<int> *>> &bc, std::list<int> neumann_labels
    ...
    const auto &Kh            = Vh.Th;
    double hi                 = Kh[0].hElement();
    double boundary_penalty   = 1000. / hi;
    double tangential_penalty = 1. / hi;
    double kappa1             = 0.5;
    double kappa2             = 0.5;
    ...
    // Matrix assembly
    stokes.addBilinear(
        contractProduct(2 * mu.expr() * Eps(u), Eps(v)) - innerProduct(p, div(v)) + innerProduct(div(u), q), Kh);

    if (Vh.basisFctType == BasisFctType::BDM1 || Vh.basisFctType == BasisFctType::RT0 ||
        Vh.basisFctType == BasisFctType::RT1) {
        stokes.addBilinear(-innerProduct(average(2 * mu.expr() * Eps(u) * t * n, kappa1, kappa2), jump(v * t)) +
                               innerProduct(jump(u * t), average(2 * mu.expr() * Eps(v) * t * n, kappa1, kappa2)) +
                               innerProduct(tangential_penalty * (jump(u * t)), jump(v * t)),
                           Kh, INTEGRAL_INNER_EDGE_2D);
    }
    ...
    if (!neumann_labels.empty()) {
        stokes.addBilinear(-innerProduct(2 * mu.expr() * Eps(u) * n, v) + innerProduct(p, v * n), Kh, INTEGRAL_BOUNDARY,
                           neumann_labels);
    }

    for (auto [gh, dirichlet_labels] : bc) {

        stokes.addBilinear(-innerProduct(2 * mu.expr() * Eps(u) * n, v) + innerProduct(p, v * n) +
                               innerProduct(boundary_penalty * u, v) + innerProduct(u, 2 * mu.expr() * Eps(v) * n),
                           Kh, INTEGRAL_BOUNDARY, *dirichlet_labels);
        stokes.addLinear(innerProduct(gh->exprList(), boundary_penalty * v) +
                             innerProduct(gh->exprList(), 2 * mu.expr() * Eps(v) * n),
                         Kh, INTEGRAL_BOUNDARY, *dirichlet_labels);
    }
