...
BoundaryDirichlet<Mesh2>::BoundaryDirichlet(const cutspace_t &Vh, const BarycentricActiveMesh2& active_mesh, const int domain) {
    ...
    for (int km = 0; km < active_mesh.active_macro_elements.size(); ++km) {
        ...
        for (int k_micro : micro_elements) {
            ...
            // a neighboring element can be 1) inside of the domain, 2) another cut element, 
            // 3) outside of the domain but in the same macro element, or 4) inside of the domain and in another macro element
                if ((kn_micro != -1) ||(active_mesh.inverse_active_macro_map[k_micro] == active_mesh.inverse_active_macro_map[kn_micro]))
                    continue;
