...
/**
 * @brief Space-time CutFEM for simulating the convection-diffusion equation on colliding circles using a quadrilateral
 *        background mesh.
 */
...
using namespace MergingQuadNS;

// Setup quadrilateral two-dimensional class types
...
#ifndef METHOD
#define METHOD conservative
#endif

#ifndef STABILIZATION
#define STABILIZATION fullstab
#endif
...
int main(int argc, char **argv) {
    ...
    if (method != "conservative" && method != "non_conservative") {
        std::cerr << "Unsupported method: " << method << "\n";
        return 1;
    }

    if (stabilization != "fullstab" && stabilization != "macro") {
        std::cerr << "Unsupported stabilization: " << stabilization << "\n";
        return 1;
    }

    std::cout << "FORCING OUTPUT NOW..." << std::endl;
    std::string path_output_figures = "/mnt/nvme0n1p4/output_figures/merging_quad/";
    ...
    for (int j = 0; j < iterations; ++j) {
        ...
        while (iter < total_number_iteration) {
            ...
            for (int n = 0; n < ndof_time_slab; n++) {
                ...
                std::transform(sol.begin(), sol.end(), u_dof_n.begin(), sol.begin(), std::plus<double>());
            }
            ...
            if (true) {
                ...
                Paraview<mesh_t> writer(Thi, path_output_figures + "merging_quad_" + std::to_string(iter + 1) + ".vtk");
